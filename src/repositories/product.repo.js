...
function create(data) {
  const stmt = db.prepare(`
    INSERT INTO products (store_id, category_id, name, slug, description, product_attributes, price, quantity, status)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
...
function update(id, data) {
  ...
  db.prepare(`
    UPDATE products
    SET category_id = ?, name = ?, slug = ?, description = ?, product_attributes = ?, price = ?, quantity = ?, status = ?
    WHERE id = ?
  `).run(
